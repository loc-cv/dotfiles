...
M.setup = function()
  ...
  fzf_lua.setup({
    -- use skim instead of fzf? (https://github.com/lotabout/skim)
    -- fzf_bin = "sk",

    -- enable global `resume`?
    -- can also be sent individually:
    -- `<any_function>.({ gl ... })`
    global_resume = true,

    -- include typed query in `resume`?
    ...
    -- provider setup
    files = {
      -- previewer      = "bat",          -- uncomment to override previewer, set to 'false' to disable
    ...
    -- Uncomment to use the old help previewer which used a minimized help window to generate the help tag preview
    -- helptags = { previewer = "help_tags" },

    -- Uncomment to use `man` command as native fzf previewer (instead of using a neovim floating window)
