...
local function get_current_file_name(readonly_icon)
  ...
  if string.len(file_readonly(readonly_icon)) ~= 0 then
    return filename .. file_readonly(readonly_icon)
  end
...
DynamicFileName = get_current_file_name
LinePercent = current_line_percent
BufferNotEmpty = buffer_not_empty
...
gls.left[6] = {
  FileName = {
    provider = function()
      return get_element(DynamicFileName())
    end,
    highlight = { active_fg, bg, "bold" },
    event = "VimResized",
  },
...
gls.right[9] = {
  PerCent = {
    provider = function()
      return get_element(LinePercent())
    end,
    highlight = { active_fg, bg, "bold" },
  },
}
